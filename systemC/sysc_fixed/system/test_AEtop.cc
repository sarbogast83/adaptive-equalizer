...
int sc_main (int argc , char *argv[]) 
{
	...
	sc_signal<sc_int<16>> dn, un;
	//sc_signal<sc_uint<5>> mu_N, M;
	sc_signal<sc_int<32>> grnd, dhatOut32; //in
    sc_signal<sc_int<16>> dhatOut, enOut; //out
	sc_signal<sc_int<16>> wn0, wn1, wn2, wn3, wn4, wn5, wn6, wn7, wn8, wn9, wn10, wn11;
	...
	u_fp = fopen("un_int.txt","r");
	d_fp = fopen("dn_int.txt","r");
	
	
	// This format for writing to a file works for outputing
	// sc_signals. Use fout instead of fprintf
	ofstream fout_w("w_int.txt");
    ofstream fout_en("en_int.txt");
    ofstream fout_dhat("dhatn_int.txt");
	...
	U_AEtop.dn(dn);
	U_AEtop.un(un);
	//U_AEtop.mu(mu); // hardcoded in AEtop
    U_AEtop.grnd(grnd);
